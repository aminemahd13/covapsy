 ...
 *
 * High-performance C implementation replacing the gap-following algorithm.
 * Designed for 1/10-scale RC car racing on closed tracks with walls.
 *
 * Algorithm: Corridor model + potential-field steering + TTC speed control
 *
...
/* ── Tuning constants (exposed for documentation; hardcoded in .c) ── */

/* Corridor centering gain: how strongly to steer toward corridor center.
 * 0.8 means full lateral offset → 80% of max steering.                  */
#define RC_K_CENTER          0.80f

/* Heading-follow gain: how strongly to chase the best open direction.    */
#define RC_K_HEADING         0.50f

/* Obstacle repulsion gain: emergency push away from very close objects.  */
#define RC_K_REPULSION       0.90f
...
#define RC_REPULSION_DIST_FACTOR  1.5f
...
#define RC_EMERGENCY_STOP_M  0.15f
 ...
 *
 *   1. Corridor model from symmetric angle-pair wall distances
 *   2. Weighted corridor centering + heading direction following
 *   3. Quadratic obstacle repulsion for emergency avoidance
 *   4. IMU yaw-rate damping for oscillation suppression
 *   5. TTC-limited speed with curvature and steering penalties
 *
